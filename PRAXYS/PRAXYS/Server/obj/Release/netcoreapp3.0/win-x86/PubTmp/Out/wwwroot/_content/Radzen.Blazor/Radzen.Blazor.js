...
var Radzen = {
    ...
    openPopup: function (e, el, popup, isDropDown, additionalStyle) {
        ...
        var style = isDropDown ? 'margin-left:-0.625rem; z-index: 1005; transform: translateY(3px); opacity: 1; display:block; position: fixed;' + Radzen.getPopupWidthAndTop(popup, isDropDown) + ';' + additionalStyle
    ...
    scrollDataGrid: function (e) {
        var popup = document.body.querySelector('.ui-overlaypanel');
        if (popup) {
            Radzen.hidePopup(popup);
        }
    ...
    getPopupWidthAndTop: function (popup, isDropDown) {
        ...
        var offsetWithPopup = el.getBoundingClientRect().bottom + popupHeight;

        if (offsetWithPopup >= windowHeight) {
        ...
        var style = 'width:calc(' + rect.width + 'px - 1.425rem); top:' + top + 'px;';
